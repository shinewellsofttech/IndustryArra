...
const Header = ({ onNote }) => {
  ...
  // Function to get role name based on F_UserType
  ...
  var filterName = name.length >= 3 ? name.filter((n, i) => i > 0) : name;
   var finalName = filterName.includes("app")
    ...
    const toggleTheme = () => {
        const nextThemeValue = background.value === 'light' ? 'dark' : 'light';
        changeBackground({ value: nextThemeValue, label: nextThemeValue === 'dark' ? 'Dark' : 'Light' });
    };
        ...
        <nav className="navbar navbar-expand">
          <div className="collapse navbar-collapse justify-content-between">
            <div className="header-left">
              ...
              <div className="user-info" style={{ padding: '10px 15px' }}>
                <span className="font-w600">
                  {getRoleName(userData?.userType)} : {userData?.name}
                </span>
              </div>
            </div>
            <ul className="navbar-nav header-right main-notification">
              ...
              <li className="nav-item">
                <button
                  onClick={toggleTheme}
                  className="nav-link ai-icon i-false c-pointer btn btn-link"
                  aria-label={background.value === 'light' ? 'Activate dark mode' : 'Activate light mode'}
