...
function ContainerEntryReport() {
  ...
  const API_URL3 = API_WEB_URLS.MASTER + "/0/token/TransferData";
  ...
  const [machineHeaderInfo, setMachineHeaderInfo] = useState({ componentName: "", containerName: "" });
  ...
  // Helpers for Machine Sequence display
  const toDisplayDateTime = (value) => {
    if (!value) return "";
    const str = String(value);
    if (str.includes('T')) return str.substring(0, 16).replace('T', ' ');
  ...
  const handleMachineSequenceOpen = useCallback(async (row) => {
    if (!row || !row.Id) return;

    const componentName = row.ComponentName || "";
    const containerName = selectedContainer?.Name || "";
    setMachineHeaderInfo({ componentName, containerName });
    ...
    <div className="container-fluid">
      ...
      >
        <ModalHeader toggle={() => setDepartmentStatusModalOpen(false)}>
          <div>
            <h5 className="mb-0">
              ...
              {selectedDepartmentName || 'Department Status'} Transfer Data
            </h5>
        ...
        <ModalBody style={{ padding: '20px' }}>
          {state.FillArray5 && state.FillArray5.length > 0 ? (
            <div className="table-responsive">
              <table className="table table-striped table-hover" style={{ marginBottom: 0 }}>
                ...
                <tbody>
                  {state.FillArray5.map((item, index) => (
                    <tr key={index}>
            ...
            <div className="text-center py-4">
              ...
              <h5>No Transfer Data Available</h5>
              <p className="text-muted">No transfer information found for this department.</p>
            </div>
          )}
        </ModalBody>
      ...
      <Modal
        ...
        toggle={() => setComponentsModalOpen(false)}
        size="xl"
        centered
        style={{ maxWidth: '98vw', width: '98vw' }}
      >
        <ModalHeader toggle={() => setComponentsModalOpen(false)}>
          <h4 className="mb-0">
                        ...
                        <tr key={idx} style={{ background: idx % 2 === 0 ? '#f8f9fa' : '#fff', borderBottom: '2px solid #e9ecef' }}>
                          ...
                          <td style={{ minWidth: 200, border: '2px solid #adb5bd' }}>
                            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
                              <div style={{ display: 'flex', alignItems: 'center', gap: 12, margin: '4px 0', minWidth: 180 }}>
                                <label style={{ fontWeight: 600, fontSize: 14, minWidth: 80 }}>Start Date</label>
                                <span style={{ fontSize: 14, color: '#495057', fontWeight: 500, whiteSpace: 'nowrap' }}>
                                  {row.WoodIssueStartDate ? row.WoodIssueStartDate.split('T')[0] : 'N/A'}
                                </span>
                              </div>
                              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                                <label style={{ fontWeight: 600, fontSize: 14, minWidth: 80 }}>End Date</label>
                                <span style={{ fontSize: 14, color: '#495057', fontWeight: 500, whiteSpace: 'nowrap' }}>
                                  {row.WoodIssueEndDate ? row.WoodIssueEndDate.split('T')[0] : 'N/A'}
                                </span>
                              </div>
                            </div>
                          </td>
                          {/* MACHINE */}
                          <td style={{ minWidth: 240, border: '2px solid #adb5bd' }}>
                            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
                              <div style={{ display: 'flex', alignItems: 'center', gap: 12, margin: '4px 0', minWidth: 220 }}>
                                <label style={{ fontWeight: 600, fontSize: 14, minWidth: 80 }}>Start Date</label>
                                <span style={{ fontSize: 14, color: '#495057', fontWeight: 500, whiteSpace: 'nowrap' }}>
                                  {row.MachineStartDate ? row.MachineStartDate.substring(0, 16) : 'N/A'}
                                </span>
                              </div>
                              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                                <label style={{ fontWeight: 600, fontSize: 14, minWidth: 80 }}>End Date</label>
                                <span style={{ fontSize: 14, color: '#495057', fontWeight: 500, whiteSpace: 'nowrap' }}>
                                  {row.MachineEndDate ? row.MachineEndDate.substring(0, 16) : 'N/A'}
                                </span>
                              </div>
                              <div style={{ display: 'flex', justifyContent: 'center', marginTop: 6 }}>
                                <button
                                  onClick={() => handleMachineSequenceOpen(row)}
                                  style={{
                                    padding: '6px 10px',
                                    background: '#0d6efd',
                                    color: '#fff',
                          ...
                          <td style={{ minWidth: 200, border: '2px solid #adb5bd' }}>
                            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
                              <div style={{ display: 'flex', alignItems: 'center', gap: 12, margin: '4px 0', minWidth: 180 }}>
                                <label style={{ fontWeight: 600, fontSize: 14, minWidth: 80 }}>Start Date</label>
                                <span style={{ fontSize: 14, color: '#495057', fontWeight: 500, whiteSpace: 'nowrap' }}>
                                  {row.ComponentStorageStartDate ? row.ComponentStorageStartDate.split('T')[0] : 'N/A'}
                                </span>
                              </div>
                              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                                <label style={{ fontWeight: 600, fontSize: 14, minWidth: 80 }}>End Date</label>
                                <span style={{ fontSize: 14, color: '#495057', fontWeight: 500, whiteSpace: 'nowrap' }}>
                                  {row.ComponentStorageEndDate ? row.ComponentStorageEndDate.split('T')[0] : 'N/A'}
                                </span>
                              </div>
                            </div>
                          </td>
                          ...
                          <td style={{ minWidth: 200, border: '2px solid #adb5bd' }}>
                            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
                              <div style={{ display: 'flex', alignItems: 'center', gap: 12, margin: '4px 0', minWidth: 180 }}>
                                <label style={{ fontWeight: 600, fontSize: 14, minWidth: 80 }}>Start Date</label>
                                <span style={{ fontSize: 14, color: '#495057', fontWeight: 500, whiteSpace: 'nowrap' }}>
                                  {row.ComponentSandingStartDate ? row.ComponentSandingStartDate.split('T')[0] : 'N/A'}
                                </span>
                              </div>
                              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                                <label style={{ fontWeight: 600, fontSize: 14, minWidth: 80 }}>End Date</label>
                                <span style={{ fontSize: 14, color: '#495057', fontWeight: 500, whiteSpace: 'nowrap' }}>
                                  {row.ComponentSandingEndDate ? row.ComponentSandingEndDate.split('T')[0] : 'N/A'}
                                </span>
                              </div>
                            </div>
                          </td>
        ...
        <ModalFooter style={{ background: '#f8f9fb', borderTop: '1px solid #e9ecef' }}>
          <Button color="secondary" onClick={() => setComponentsModalOpen(false)} style={{ minWidth: 100, fontWeight: 600, fontSize: 15 }}>
            <i className="fas fa-times me-2"></i>Cancel
          </Button>
        </ModalFooter>
      </Modal>

      {/* Machine Sequence Modal (read-only) */}
      <Modal
      ...
      >
        <ModalHeader toggle={toggleMachineSequenceModal}>
          {machineHeaderInfo.componentName || 'Component'} | {machineHeaderInfo.containerName || 'Container'}
        </ModalHeader>
        <ModalBody>
          {machineSequenceLoading ? (
            ...
            <div className="table-responsive">
              <table className="table table-striped table-hover align-middle">
                ...
                <tbody>
                  {(state.MachineSequenceData || []).length > 0 ? (
                    state.MachineSequenceData.map((seq, i) => (
                      <tr key={seq.Id || i}>
                        ...
                        <td style={{ textAlign: 'center' }}>
                          <span>{toDisplayDateTime(seq.StartDate) || 'N/A'}</span>
                        </td>
                        <td style={{ textAlign: 'center' }}>
                          <span>{toDisplayDateTime(seq.EndDate) || 'N/A'}</span>
                        </td>
                      </tr>
                    ))
                  ) : (
...
export default ContainerEntryReport;
