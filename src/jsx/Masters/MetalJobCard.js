...
const fetchData = async () => {
  ...
  // Handle individual job card selection
  ...
  const isAllSelected = jobCardArray.length > 0 && selectedJobCardIds.length === jobCardArray.length;
  ...
  const handlePrintPDF = async () => {
    try {
      ...
      const selectedIndexes = jobCardArray
        .map((card, idx) => (selectedJobCardIds.includes(card.ID) ? idx : -1))
        .filter((idx) => idx !== -1);
      ...
      setPdfProgress({ current: 0, total: selectedIndexes.length });
      ...
      for (let i = 0; i < selectedIndexes.length; i++) {
        ...
        const cardIdx = selectedIndexes[i];
        const canvas = await html2canvas(jobCards[cardIdx], {
          scale: 2,
          useCORS: true,
          logging: true,
          backgroundColor: '#ffffff',
          width: jobCards[cardIdx].offsetWidth,
          height: jobCards[cardIdx].offsetHeight,
          onclone: (document) => {
            const element = document.getElementsByClassName('job-card')[cardIdx];
            if (element) {
        ...
        ${jobCards.map(jobCard => {
          ...
          return `
            <div class="job-card" data-machine-rows="${machineDetails.length}">
              ...
              <table>
                <tr class="info-row">
                  <td class="label" style="width: 25%">WOOD MACHINE CENTRE</td>
                  <td class="label" style="width: 15%">JOB CARD</td>
                  <td class="label" style="width: 10%">NO.</td>
                  <td style="width: 50%">${jobCard.JobCardNo || 'N/A'}</td>
                </tr>
              </table>
              ...
              <table>
                ...
                <tr class="info-row">
                  ...
                  <td colspan="2">${jobCard.ItemName || 'N/A'}</td>
                  <td class="label">ORDER QTY: ${jobCard.OrderQty || 'N/A'}</td>
                </tr>
                <tr class="info-row">
                  <td class="label">BATCH CODE OF MATERIAL</td>
                  <td>${jobCard.BatchCode || ''}</td>
                  <td class="label">BATCH CODE OF POWDER</td>
                  <td>${jobCard.PowderBatchCode || ''}</td>
                </tr>
                <tr class="info-row">
              ...
              <table class="dimensions-table">
                <tr>
                  <th colspan="4" class="label">Final Dimension (in mm)</th>
                </tr>
                <tr>
  ...
  // Add dummy machine array data for each job card
 

  const handleBatchCodeChange = async (e, jobCardId, field) => {
    try {
      const newBatchCode = e.target.value;
      // Update local state
      const updatedJobCards = jobCardArray.map(card => 
        card.ID == jobCardId 
          ? { ...card, [field]: newBatchCode } 
          : card
      );
  ...
  const handleBulkBatchCodeChange = (e, field) => {
    const newBatchCode = e.target.value;
  ...
  const handleBatchCodeSubmit = async (batchCodesArray) => {
    try {
      ...
      let vformData = new FormData();
      // Convert array to JSON string containing ID, BatchCode, and PowderBatchCode
      const batchCodeData = batchCodesArray.map(item => ({
        Id: item.ID,
        BatchCode: item.BatchCode || '',
        BatchCodePowder: item.PowderBatchCode || ''
      }));
  ...
  const handleBulkBatchCodeSubmit = async () => {
    try {
      // Filter and create array of job cards that have either BatchCode or PowderBatchCode
      const batchCodesArray = jobCardArray
        .filter(card => 
          (card.BatchCode && card.BatchCode.trim() !== '') || 
          (card.PowderBatchCode && card.PowderBatchCode.trim() !== '')
        );
    ...
    <Row className="mb-3" style={{ padding: "0 20px" }}>
      <Col xs={12} md={6}>
        <div style={{
          ...
          >
            Bulk Material Batch Code:
          </label>
          <input
            id="bulkMaterialBatchCode"
            type="text"
            onChange={(e) => handleBulkBatchCodeChange(e, 'BatchCode')}
            style={{
              padding: '0.5rem',
              border: '1px solid #000000',
              borderRadius: '4px',
      ...
      <Col xs={12} md={6}>
        <div style={{
          ...
          >
            Bulk Powder Batch Code:
          </label>
          <input
            id="bulkPowderBatchCode"
            type="text"
            onChange={(e) => handleBulkBatchCodeChange(e, 'PowderBatchCode')}
            style={{
              padding: '0.5rem',
              border: '1px solid #000000',
              borderRadius: '4px',
    ...
    <Row className="mb-3" style={{ padding: "0 20px" }}>
      <Col>
        ...
        <div style={{ marginBottom: '10px' }}>
          ...
          />
          <label htmlFor="selectAllJobCards" style={{ fontWeight: 700 }}>
...
>
      {jobCardArray.map((jobCard) => (
        ...
        >
          ...
          {/* Wood Machine Centre & Job Card No Row */}
          <Row className="g-0">
            <Col xs={4} style={{
          ...
          <Row className="g-0">
            ...
            <Col xs={4} style={{
              ...
              <input
                ...
                onChange={(e) => handleBatchCodeChange(e, jobCard.ID, 'BatchCode')}
                style={{
                  width: "100%",
                  padding: "0.5rem",
                  border: "1px solid #000000",
                ...
                placeholder="Enter material batch code"
              />
            </Col>
            <Col xs={2} style={{
              borderRight: "1px solid #000000",
            ...
            <Col xs={2} style={{
              ...
              <input
                type="text"
                value={jobCard.PowderBatchCode || ""}
                onChange={(e) => handleBatchCodeChange(e, jobCard.ID, 'PowderBatchCode')}
                style={{
                  width: "100%",
                  padding: "0.5rem",
                  border: "1px solid #000000",
                ...
                placeholder="Enter powder batch code"
              />
            </Col>
          </Row>
