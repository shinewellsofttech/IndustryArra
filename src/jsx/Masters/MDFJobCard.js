...
const MDFJobCard = ({
  ...
  const API_URL_UPDATE_REQ = `${API_WEB_URLS.MASTER}/0/token/UpdateReq`;
    ...
    const fetchDrawingsForJobCards = async () => {
      ...
      for (const jobCard of jobCardArray) {
        if (
          ...
          !drawingsData[jobCard.F_ComponentsMaster]
        ) {
          setDrawingsLoading((prev) => ({
            ...prev,
            [jobCard.F_ComponentsMaster]: true,
          }));
          ...
          try {
            const result = await Fn_FillListData(
              ...
              setState,
              "FillArray5",
              `${API_URL_DRAWINGS}/Id/${jobCard.F_ComponentsMaster}`
            );
            ...
            console.error(
              `Error fetching drawings for component ${jobCard.F_ComponentsMaster}:`,
              error
            );
            setDrawingsData((prev) => ({
              ...prev,
            ...
            setDrawingsLoading((prev) => ({
              ...prev,
              [jobCard.F_ComponentsMaster]: false,
            }));
          }
        }
      }
  ...
  // Handle Qty2 change for job cards
  const handleQty2Change = (jobCardId, value) => {
    const updatedJobCards = jobCardArray.map((card) =>
      card.ID === jobCardId ? { ...card, Qty2: value } : card
    );
    setJobCardArray(updatedJobCards);
  };

  // Handle Enter key press for Qty2
  const handleQty2Enter = async (jobCardId, value) => {
    const jobCard = jobCardArray.find((card) => card.ID === jobCardId);
    console.log("Job:", jobCard);
    console.log("Qty2 Value:", value);
   const res =  await Fn_FillListData(dispatch, setState, "Nothing", `${API_URL_UPDATE_REQ}/${value}/${jobCardId}`);
  ...
  const getMachineDetails = (jobCardId) => {
    return machineArray.filter(
      (machine) => machine.F_JobCardMaster === jobCardId
    );
  };
  ...
  const generatePrintHTML = (selectedJobCards) => {
    const jobCards = jobCardArray.filter((card) =>
      selectedJobCards.includes(card.ID)
    );
      ...
      <head>
        <style>
          ...
          @media print {
            ...
            .job-card[data-machine-rows="6"] .drawing-section { height: 90mm; }
            .job-card[data-machine-rows="7"] .drawing-section { height: 80mm; }
            .job-card[data-machine-rows="8"] .drawing-section { height: 70mm; }
            .job-card[data-machine-rows="9"] .drawing-section { height: 60mm; }
            .job-card[data-machine-rows="10"] .drawing-section { height: 50mm; }
            ...
            .job-card[data-machine-rows="6"] .machine-table-section { height: 75mm; }
            .job-card[data-machine-rows="7"] .machine-table-section { height: 80mm; }
            .job-card[data-machine-rows="8"] .machine-table-section { height: 85mm; }
            .job-card[data-machine-rows="9"] .machine-table-section { height: 90mm; }
            .job-card[data-machine-rows="10"] .machine-table-section { height: 95mm; }
          ...
          .map((jobCard) => {
            ...
            <div class="job-card" data-machine-rows="${machineDetails.length}">
              ...
              <div class="drawing-section">
                ...
                ${
                  drawingsData[jobCard.F_ComponentsMaster] &&
                  drawingsData[jobCard.F_ComponentsMaster].length > 0
                    ? `
                  <div style="display: flex; flex-wrap: wrap; justify-content: center; gap: 2mm; width: 100%;">
                    ${drawingsData[jobCard.F_ComponentsMaster]
                      .map((drawing, index) => {
                        const imageCount =
                          drawingsData[jobCard.F_ComponentsMaster].length;
                        let imageSize, containerSize;

                        if (imageCount === 1) {
                          imageSize = "max-width: 120mm; max-height: 80mm;";
                          containerSize =
                            "max-width: 130mm; min-width: 100mm; width: 100%;";
                        } else if (imageCount === 2) {
                          imageSize = "max-width: 65mm; max-height: 50mm;";
                          containerSize =
                            "max-width: 75mm; min-width: 55mm; width: 48%;";
                        } else if (imageCount === 3) {
                          imageSize = "max-width: 50mm; max-height: 38mm;";
                          containerSize =
                            "max-width: 60mm; min-width: 40mm; width: 32%;";
                        } else if (imageCount === 4) {
                          imageSize = "max-width: 40mm; max-height: 30mm;";
                          containerSize =
                            "max-width: 50mm; min-width: 30mm; width: 24%;";
                        } else {
                          imageSize = "max-width: 32mm; max-height: 24mm;";
                          containerSize =
                            "max-width: 40mm; min-width: 25mm; width: 20%;";
                        }
                ...
                `
                    : '<div style="height: 50mm; display: flex; align-items: center; justify-content: center; color: #666;">No drawings available</div>'
                }
              </div>
        ...
        >
          ...
          >
            ...
            >
              Generating PDF...
              <br />
              Processing Job Card {pdfProgress.current} out of{" "}
              {pdfProgress.total}
              <br />
              <div
                style={{
                ...
                <Row className="g-0">
                  ...
                  >
                    <input
                      type="text"
                      value={jobCard.Qty2 || ""}
                      onChange={(e) => handleQty2Change(jobCard.ID, e.target.value)}
                      onKeyDown={(e) => {
                        if (e.key === "Enter") {
                          handleQty2Enter(jobCard.ID, e.target.value);
                        }
                      }}
                      style={{
                        width: "100%",
                ...
                >
                  ...
                  >
                    {drawingsLoading[jobCard.F_ComponentsMaster] ? (
                      ...
                      >
                        {drawingsData[jobCard.F_ComponentsMaster].map(
                          (drawing, index) => {
                            const imageCount =
                              drawingsData[jobCard.F_ComponentsMaster].length;
                            let imageSize, containerSize;
                              ...
                              >
                                <img
                                  ...
                                  <html>
                                    ...
                                    <body>
                                      <img src="${API_WEB_URLS.IMAGEURL}/${
                                      drawing.ImageDataNew
                                    }" alt="${
                                      drawing.Name || `Drawing ${index + 1}`
                                    }" />
                                    </body>
